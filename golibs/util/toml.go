...
// }
func ParseTOMLConfig(fileName string, configObj interface{}) error {
	registerTOMLConfig("", configObj)
	return config.Parse(fileName)
}

func UnmarshalTomlStr(content string, configObj interface{}) error {
	return toml.Unmarshal([]byte(content), configObj)
}
func UnmarshalToml(fileName string, configObj interface{}) error {
	content, err := ioutil.ReadFile(fileName)
	if err != nil {
		return err
	}
	return toml.Unmarshal(content, configObj)
}
