...
func (this FieldBoss) MarshalJSON() ([]byte, error) {
	...
	cdStr += "]"
	buf.WriteString(fmt.Sprintf(`[%d,%d,%d,%s]`, this.DareNum, this.BuyNum, this.ResetTime, cdStr))
	return buf.Bytes(), nil
}
...
func (this ExpStage) MarshalJSON() ([]byte, error) {
	...
	expStagesStr += "]"
	buf.WriteString(fmt.Sprintf(`[%d,%d,%d,%s]`, this.DareNum, this.BuyNum, this.ResetTime, expStagesStr))
	return buf.Bytes(), nil
}
...
func (this *ExpStage) UnMarshalJSON(data []byte) error {
	...
	for _, v := range expStagesInterface {
		...
		stageId := int(expStageArr[0].(float64))
		exp := int(expStageArr[1].(float64))
		expStagesMap[stageId] = exp
	}
...
func (this MainLineTask) Value() (driver.Value, error) {
	return fmt.Sprintf("[%d,%d]", this.TaskId, this.Process), nil
}
...
func (this DayStateRecord) MarshalJSON() ([]byte, error) {
	...
	buf.WriteString(fmt.Sprintf(`[%d,%d,%s]`, this.Day, this.RankWorship, monthCardStr))
	return buf.Bytes(), nil
}
...
func (this Wear) MarshalJSON() ([]byte, error) {
	...
	atlasStr += "]"
	buf.WriteString(fmt.Sprintf(`[%d,%d,%d,%s,%d]`, this.FashionWeaponId, this.FashionClothId, this.WingId, atlasStr, this.MagicCircleLvId))
	return buf.Bytes(), nil
}
...
func (this Sign) MarshalJSON() ([]byte, error) {
	...
	cumulativeStr += "]"
	buf.WriteString(fmt.Sprintf(`%d,%s,%d,%s`, this.Count, signDayStr, this.ResetTime, cumulativeStr))
	buf.WriteByte(']')
	return buf.Bytes(), nil
}
...
func (this MaterialStage) MarshalJSON() ([]byte, error) {
	...
	dareStr += "]"
	sweepStr := "["
	if len(this.SweepNum) > 0 {
		for stageId, num := range this.SweepNum {
			sweepStr += fmt.Sprintf(`[%d,%d],`, stageId, num)
		}
		sweepStr = sweepStr[:len(sweepStr)-1]
	}
	sweepStr += "]"
	buf.WriteString(fmt.Sprintf(`[%s,%d,%s]`, dareStr, this.ResetTime, sweepStr))
	return buf.Bytes(), nil
}
...
func (this *MaterialStage) UnmarshalJSON(data []byte) error {
	...
	sweepMap := make(IntKv)
	if len(datas) > 2 {
		sweepInterface := datas[2].([]interface{})
		for _, sweep := range sweepInterface {
			sweepArr := sweep.([]interface{})
			sweepMap[int(sweepArr[0].(float64))] = int(sweepArr[1].(float64))
		}
	}
	(*this).DareNum = dareMap
	(*this).ResetTime = int(datas[1].(float64))
	(*this).SweepNum = sweepMap
	return nil
}
...
func (this DailyTaskInfo) MarshalJSON() ([]byte, error) {
	...
	activityStr += "]"
	buf.WriteString(fmt.Sprintf(`[%d,%d,%d,%s,[%s],[%s],%d]`, this.DayExp, this.WeekExp, this.ResourcesBackExp, activityStr, common.JoinIntSlice(this.GetDayRewardIds, ","), common.JoinIntSlice(this.GetWeekRewardIds, ","), this.ResetTime))
	return buf.Bytes(), nil
}
...
func (this FirstRecharge) MarshalJSON() ([]byte, error) {
	...
	buf.WriteString(fmt.Sprintf(`[%d,%s,%d]`, isRecharge, dayStr, this.OpenDay))
	return buf.Bytes(), nil
}
...
func (this SpendRebates) MarshalJSON() ([]byte, error) {
	...
	rewardsStr += "]"
	buf.WriteString(fmt.Sprintf(`[%d,%d,%s]`, this.CountIngot, this.Ingot, rewardsStr))
	return buf.Bytes(), nil
}
...
func (this LimitGift) MarshalJSON() ([]byte, error) {
	...
	listStr += "]"
	buf.WriteString(fmt.Sprintf(`[%s,%s,%s,%s]`, gradeStatusStr, isBuyStr, tlvStr, listStr))
	return buf.Bytes(), nil
}
...
func (this DailyPack) MarshalJSON() ([]byte, error) {
	...
	for t, dailyPack := range this {
		...
		buyIdsStr += "]"
		if buf.Len() == 1 {
			buf.WriteString(fmt.Sprintf(`[%d,%s,%d]`, t, buyIdsStr, dailyPack.ResetTime))
		} else {
			buf.WriteString(fmt.Sprintf(`,[%d,%s,%d]`, t, buyIdsStr, dailyPack.ResetTime))
		}
	}
...
func (this Elf) MarshalJSON() ([]byte, error) {
	...
	skillBagStr += "]"
	buf.WriteString(fmt.Sprintf(`[%d,%d,%s,%s]`, this.Lv, this.Exp, skillStr, skillBagStr))
	return buf.Bytes(), nil
}
...
func (this CardInfo) MarshalJSON() ([]byte, error) {
	...
	buf.WriteString(fmt.Sprintf(`["%v","%v","%v","%v","%v"]`, this.AddWeight, this.DrawTimes, this.Season, this.Integral, getAwardIds))
	return buf.Bytes(), nil
}
...
func (this TreasureInfo) MarshalJSON() ([]byte, error) {
	...
	haveRandomItems += "]"
	buf.WriteString(fmt.Sprintf(`[%v,%v,%v,%v,[%v],%v,%v,%v]`, this.Season, this.PopUpState, this.PopUpResOpenDay, this.AllUseTimes, common.JoinIntSlice(this.AllGetRound, ","), buyStr, chooseItems, haveRandomItems))
	return buf.Bytes(), nil
}
...
func (this CompetitiveInfo) MarshalJSON() ([]byte, error) {
	...
	buf.WriteString(fmt.Sprintf(`["%v","%v","%v"]`, this.HaveChallengeTimes, this.BuyTimes, this.DayResDay))
	return buf.Bytes(), nil
}
